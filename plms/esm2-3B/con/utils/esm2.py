...
class ESM2(nn.Module):
    ...
    def _init_layer_devices(self):
        try:
            self.layer_devices = [list(layer.parameters())[0].device for layer in self.layers]
        except:
            self.layer_devices=None
    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=False, return_contacts=False):
        ...
        if self.layer_devices:
            if x.device != list(self.emb_layer_norm_after.parameters())[0].device:
                x = x.to(list(self.emb_layer_norm_after.parameters())[0].device)
