...
def construct_tree(node_name, module_graph, name_layer_map, path_visited):
    if node_name in path_visited:
        raise ValueError(f"Circular reference detected at {node_name}")
    path_visited.add(node_name)

    node = TreeNode(node_name)
    layer = name_layer_map.get(node_name, None)
    node.layer = layer

    # print(f"Constructing tree node for: {node_name}")

    if node_name in module_graph:
        for child_name in module_graph[node_name]:
            if child_name not in path_visited:
                child_node = construct_tree(child_name, module_graph, name_layer_map, path_visited.copy())
...
def partition_layers(node, n, e=0.5):
    partition_amt = [0 for _ in range(n)]
    print("partition_amt:", partition_amt)
    ...
    while children:
        child = children[0]
        if current_partition >= len(partition_amt):
            print(f'Error: current_partition = {current_partition}, len(partition_amt) = {len(partition_amt)}')
            raise IndexError("No more partitions available.")
